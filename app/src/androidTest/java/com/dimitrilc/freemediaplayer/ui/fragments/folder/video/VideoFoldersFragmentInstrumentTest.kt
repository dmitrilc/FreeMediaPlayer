...
class VideoFoldersFragmentInstrumentTest{
    private val foldersFullCount = 2
    private val firstFoldersRelativeCount = 4
    private val secondFoldersRelativeCount = 3

    @get:Rule
    val activityRule = ActivityScenarioRule(MainActivity::class.java)

    @Before
    fun navigateToVideoFoldersPath(){
        /*
        Because fragment ID and nav Button ID are the same. Click on Video button first
        to switch to video destination. This is more organic than navigating with
        NavController
        */
    ...
    @Test
    fun test_isVideoFoldersCurrentDestination_onStart(){
        activityRule.scenario.onActivity {
            val navController = it.findNavController(R.id.fragmentContainerView_navHostFragment)
            val currentDestinationId = navController.currentDestination?.id
            assertEquals(currentDestinationId, R.id.video_folders_path)
        }
    }
